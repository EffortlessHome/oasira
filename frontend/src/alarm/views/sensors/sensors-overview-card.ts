...
import { TableColumn, TableData, TableFilterConfig } from '../../components/effortlesshome-table';
...
import { effortlesshomeArea, effortlesshomeSensor, Dictionary, EArmModes, HomeAssistant } from '../../types';
...
export class SensorsOverviewCard extends SubscribeMixin(LitElement) {
  ...
  @property()
  areas!: Dictionary<effortlesshomeArea>;

  @property()
  sensors!: Dictionary<effortlesshomeSensor>;
  ...
  private tableColumns(): Dictionary<TableColumn> {
    ...
    return {
      icon: {
        width: '40px',
        renderer: (data: effortlesshomeSensor) => {
      ...
      name: {
        ...
        grow: true,
        text: true,
        renderer: (data: effortlesshomeSensor & { name: string }) => html`
      ...
      modes: {
        ...
        hide: this.narrow,
        text: true,
        renderer: (data: effortlesshomeSensor) => html`
          ${data.area == noArea ? warningTooltip() : ''}
  ...
  private getTableFilterOptions() {
    let areaFilterOptions = Object.values(this.areas)
      .map(e =>
        Object({
          value: e.area_id,
          name: e.name,
          badge: (list: effortlesshomeSensor[]) => list.filter(item => item.area == e.area_id).length,
        })
      )
      ...
      areaFilterOptions = [
        {
          ...
          badge: (list: effortlesshomeSensor[]) => list.filter(item => item.area == noArea).length,
        },
        ...areaFilterOptions,
      ];
    ...
    const modeFilterOptions = getModesList(this.areas).map(e =>
      Object({
        ...
        badge: (list: effortlesshomeSensor[]) => list.filter(item => item.modes.includes(e)).length,
      })
    );
