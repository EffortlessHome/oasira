...
class GalleryCard extends LitElement {
  ...
  render() {
    const menuAlignment = (
      this.config.menu_alignment || "responsive"
    ).toLowerCase();
      ...
      <ha-card .header=${this.config.title} class="menu-${menuAlignment}">
        ...
        !(this.config.enable_date_search ?? false)
          ...
          : html`<input
              ...
              value="${this._formatDateForInput(this.selectedDate)}"
            />`}
        ${this.currentResourceIndex == undefined ||
        !(this.config.show_reload ?? false)
          ...
          : html`<ha-progress-button
              class="btn-reload"
              @click="${(ev) => this._loadResources(this._hass)}"
              >Reload</ha-progress-button
            >`}
        <div
          class="resource-viewer"
          @touchstart="${(ev) => this._handleTouchStart(ev)}"
          @touchmove="${(ev) => this._handleTouchMove(ev)}"
        >
          <figure style="margin:5px;">
            ${this._currentResource().isHass
              ...
              ? html`<img
                  ...
                  src="${this._currentResource().url}"
                />`
              : html`<video
                  controls
                  ?loop=${this.config.video_loop}
                  ?autoplay=${this.config.video_autoplay}
                  src="${this._currentResource().url}#t=0.1"
                  @loadedmetadata="${(ev) => this._videoMetadataLoaded(ev)}"
                  @canplay="${(ev) => this._startVideo(ev)}"
                  preload="metadata"
                ></video>`}
            <figcaption>
              ${this._currentResource().caption}
              ${this._isImageExtension(this._currentResource().extension)
                ? html``
                : html`<span class="duration"></span> `}
              ${!(this.config.show_zoom ?? false)
                ? html``
                : html`<a href="${this._currentResource().url}" target="_blank"
                    >Zoom</a
                  >`}
            </figcaption>
          </figure>
          ...
          ${this.resources.map((resource, index) => {
            return html`
              <figure
                ...
                @click="${(ev) => this._selectResource(index)}"
                class="${index == this.currentResourceIndex ? "selected" : ""}"
              >
                ${resource.isHass
                  ? html`<hui-image
                      .hass=${this._hass}
                  ...
                  ? html`<img
                      class="lzy_img"
                      src="/local/community/gallery-card/placeholder.jpg"
                      data-src="${resource.url}"
                    />`
                  : this.config.video_preload ?? true
                  ? html`<video
                      preload="none"
                      data-src="${resource.url}#t=${this.config
                        .preview_video_at == null
                        ? 0.1
                        : this.config.preview_video_at}"
                      @loadedmetadata="${(ev) => this._videoMetadataLoaded(ev)}"
                      @canplay="${(ev) => this._downloadNextMenuVideo()}"
                      preload="metadata"
                    ></video>`
                  : html`<center>
                      <div class="lzy_img">
                ...
                <figcaption>
                  ${resource.caption} <span class="duration"></span>
                </figcaption>
              </figure>
            `;
          })}
  ...
  updated(changedProperties) {
    ...
    if (!this._downloadingVideos) this._downloadNextMenuVideo();
  }

  async _downloadNextMenuVideo() {
    this._downloadingVideos = true;
    let v = this.shadowRoot.querySelector(
      ".resource-menu figure video[data-src]",
    );
  ...
  setConfig(config) {
    ...
    if (this._hass !== undefined) this._loadResources(this._hass);
  ...
  set hass(hass) {
    ...
    if (this.resources == null) this._loadResources(this._hass);
  }
  ...
  _doSlideShow(firstTime) {
    if (!firstTime) this._selectResource(this.currentResourceIndex + 1, true);
  ...
  _selectResource(idx, fromSlideshow) {
    ...
    if (
      ...
      this.parentNode.tagName.toLowerCase() == "hui-card-preview"
    ) {
      return;
    }

    var elt = this.shadowRoot.querySelector(
      "#resource" + this.currentResourceIndex,
    );
    if (elt)
      elt.scrollIntoView({
        behavior: "smooth",
  ...
  _startVideo(evt) {
    if (this.autoPlayVideo) evt.target.play();
  }
  ...
  _videoMetadataLoaded(evt) {
    ...
    if (!isNaN(parseInt(evt.target.duration)) && showDuration)
      evt.target.parentNode.querySelector(".duration").innerHTML =
        "[" + this._getFormattedVideoDuration(evt.target.duration) + "]";

    if (this.config.video_muted) evt.target.muted = "muted";
  }
  ...
  _loadResources(hass) {
    ...
    const maximumFiles = maximumFilesPerEntity
      ? this.config.maximum_files
      : undefined;
    const maximumFilesTotal = maximumFilesPerEntity
      ? undefined
      : this.config.maximum_files;
    ...
    this.config.entities.forEach((entity) => {
      ...
      if (typeof entity === "object") {
        ...
        if (entity.folder_format) folderFormat = entity.folder_format;
        if (entity.file_name_format) fileNameFormat = entity.file_name_format;
        if (entity.file_name_date_begins)
          fileNameDateBegins = entity.file_name_date_begins;
        if (entity.caption_format) captionFormat = entity.caption_format;
      } else {
        entityId = entity;
      }
      ...
      if (entityId.substring(0, 15).toLowerCase() == "media-source://") {
        ...
        if (entityState == undefined) {
          ...
          if (entityState.attributes.fileList != undefined)
            commands.push(
              this._loadFilesResources(
                entityState.attributes.fileList,
                maximumFiles,
                fileNameFormat,
                fileNameDateBegins,
                captionFormat,
          ...
          if (entityState.attributes.file_list != undefined)
            commands.push(
              this._loadFilesResources(
                entityState.attributes.file_list,
                maximumFiles,
                fileNameFormat,
                fileNameDateBegins,
                captionFormat,
    ...
    Promise.all(commands).then((resources) => {
      ...
      if (parsedDateSort) {
        if (reverseSort) {
          this.resources.sort(function (x, y) {
      ...
      if (randomSort) {
        for (var i = this.resources.length - 1; i > 0; i--) {
          ...
          if (i != r) {
            [this.resources[i], this.resources[r]] = [
              this.resources[r],
              this.resources[i],
            ];
          }
      ...
      if (
        maximumFilesTotal != undefined &&
        !isNaN(maximumFilesTotal) &&
        maximumFilesTotal < this.resources.length
      ) {
        //Keep only N total, but make sure camera resources remain
        this.resources = this.resources.filter(
          function (resource) {
            if (resource.isHass) return true;
      ...
      resources
        ...
        .forEach((error) => {
          ...
          this._hass.callService("system_log", "write", {
            message:
              "Gallery Card Error:  " + error.message + "   " + error.entity,
          });
        });
    });
  }
    ...
    return new Promise(async (resolve, reject) => {
      ...
      try {
        ...
        if (
          folderFormat &&
          reverseSort &&
          maximumFiles != undefined &&
          !isNaN(maximumFiles)
        ) {
          //Can do more targeted folder searching under these conditions
          var date = dayjs();
          var folderPrev = "";
          var failedPaths = [];
          ...
          while (values.length < maximumFiles) {
            ...
            if (folder != folderPrev) {
              try {
                var folderValues = await this._loadMedia(
                  this,
                  hass,
                  mediaPath,
                  maximumFiles,
            ...
            if (failedPaths.length > 2) {
              if (values.length == 0) {
                ...
                throw {
                  message:
                    "Failed to browse several folders and found no media files.  Verify your settings are correct.",
                };
              }
              break;
            }
        ...
        values.forEach((mediaItem) => {
          var resource = this._createFileResource(
            mediaItem.authenticated_path,
            fileNameFormat,
            fileNameDateBegins,
            captionFormat,
          );
  ...
  _loadMedia(
    ...
    if (
      contentId.substring(contentId.length - 1, contentId.length) != "/" &&
      contentId != "media-source://media_source"
    ) {
    ...
    return Promise.all(
      ...
      if (reverseSort) mediaItems.reverse();

      if (
        maximumFiles != undefined &&
        !isNaN(maximumFiles) &&
        maximumFiles < mediaItems.length
      ) {
      ...
      return Promise.all(
        mediaItems.map(function (mediaItem) {
          return ref
            ._fetchMediaItem(hass, mediaItem.media_content_id)
            .then(function (auth) {
              return {
                ...mediaItem,
      ...
      if (mediaItem.children) {
        commands.push(
          ...mediaItem.children
            .filter((mediaItem) => {
              return (
                ((includeVideo && mediaItem.media_class == "video") ||
                  (includeImages && mediaItem.media_class == "image") ||
                  (recursive &&
                    mediaItem.media_class == "directory" &&
                    (!filterForDate ||
                      mediaItem.title ==
                        ref._folderDateFormatter(
                          ref.config.search_date_folder_format == null
                            ? "DD_MM_YYYY"
                            : ref.config.search_date_folder_format,
                          ref.selectedDate,
                        )))) &&
        ...
        commands.push(
          ref
            ._fetchMediaContents(hass, mediaItem.media_content_id)
            .then((mediaItem) => {
              return Promise.all(
                ref._fetchMedia(
                  ref,
    ...
    if (files) {
      ...
      files.forEach((file) => {
        ...
        if (filePath.indexOf("/config/www/") < 0)
          fileUrl =
            "/local/" + filePath.substring(filePath.indexOf("/www/") + 5);
  ...
  _createFileResource(
    ...
    if (fileName != "@eaDir") {
      ...
      if (captionFormat != " ") fileCaption = fileName;
      ...
      if (fileNameFormat) date = dayjs(fileDatePart, fileNameFormat);
      ...
      if (date && captionFormat) {
        if (captionFormat.toUpperCase().trim() == "AGO")
          ...
          fileCaption = fileCaption.replace(/ago/gi, date.fromNow());
        }
      }
...
console.groupCollapsed(
  `%cGALLERY-CARD ${GalleryCardVersion} IS INSTALLED`,
...
window.customCards.push({
  ...
  description:
    "The Gallery Card allows for viewing multiple images/videos.  Requires the Files sensor availble at https://github.com/TarheelGrad1998", // Optional
});
