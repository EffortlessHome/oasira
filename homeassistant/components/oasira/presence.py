...
class PresenceBinarySensor(
    ...
    def __init__(self, hass, auto_area: AutoArea) -> None:
        ...
        _LOGGER.debug("Presence entities %s", self.entity_ids)

    @override
    @cached_property
    def unique_id(self) -> str | None:
        """Return a unique ID."""
        return f"{self.auto_area.area_id}_aggregated_presence"
    ...
    async def async_added_to_hass(self):
        ...
        _LOGGER.info("%s: Initial presence %s", self.auto_area.area_name, self.presence)
    ...
    async def _handle_state_change(self, event: Event[EventStateChangedData]) -> None:
        ...
        _LOGGER.debug("presence sensor handling state change %s", to_state)
        ...
        if current_state in PRESENCE_ON_STATES:
            if not self.presence:
                _LOGGER.info("%s: Presence detected", self.auto_area.area_name)
                self.presence = True
                self.schedule_update_ha_state()
        elif all_states_are_off(
            self.hass,
            self.entity_ids,
            PRESENCE_ON_STATES,
        ):
            if self.presence:
                _LOGGER.info("%s: Presence cleared", self.auto_area.area_name)
                self.presence = False
                self.schedule_update_ha_state()
