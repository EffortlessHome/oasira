...
from .device_binding import (
    attach_entities_to_source_device,
    bind_config_entry_to_device,
)
from .errors import (
    PowercalcSetupError,
...
FILTER_CONFIG = vol.Schema(
    {
        ...
        vol.Optional(CONF_DOMAIN): vol.Any(
...
def convert_config_entry_to_sensor_config(
    config_entry: ConfigEntry, hass: HomeAssistant
) -> ConfigType:  # noqa: C901
    ...
    def process_states_power(states_power: dict) -> dict:
        ...
        return {
            key: Template(value) if isinstance(value, str) and "{{" in value else value
    ...
    def process_fixed_config() -> None:
        ...
        if CONF_STATES_POWER in fixed_config:
            fixed_config[CONF_STATES_POWER] = process_states_power(
    ...
    def process_linear_config() -> None:
        ...
        if CONF_CALIBRATE in linear_config:
            linear_config[CONF_CALIBRATE] = process_calibrate(
                linear_config[CONF_CALIBRATE]
            )
    ...
    def process_utility_meter_offset() -> None:
        if CONF_UTILITY_METER_OFFSET in sensor_config:
            sensor_config[CONF_UTILITY_METER_OFFSET] = timedelta(
                days=sensor_config[CONF_UTILITY_METER_OFFSET]
            )
...
async def create_sensors(
    ...
    if is_individual_sensor_setup(config):
        return await setup_individual_sensors(
            hass, config, global_config, config_entry, context
        )

    sensor_configs: dict[str, ConfigType] = {}
    entities_to_add = EntitiesBucket()

    await setup_nested_or_group_sensors(
        hass, config, context, entities_to_add, sensor_configs
    )

    await add_discovered_entities(hass, config, entities_to_add, sensor_configs)

    await create_entities_sensors(
        hass,
        config,
        global_config,
        context,
...
async def setup_individual_sensors(
    ...
    if sensor_type == SensorType.GROUP:
        return EntitiesBucket(
            new=await create_group_sensors(hass, merged_sensor_config, config_entry)
        )

    return await create_individual_sensors(
        hass, merged_sensor_config, context, config_entry
    )
...
async def add_discovered_entities(
    ...
    if CONF_INCLUDE in config:
        ...
        found_entities, discoverable_entities = await resolve_include_entities(
            hass, include_config
        )
...
async def create_individual_sensors(
    ...
    energy_sensor = await handle_energy_sensor_creation(
        hass, sensor_config, source_entity, entities_to_add
    )

    if not energy_sensor:
        try:
            power_sensor = await create_power_sensor(
                hass, sensor_config, source_entity, config_entry
            )
        except PowercalcSetupError:
            return EntitiesBucket()
        entities_to_add.append(power_sensor)
        energy_sensor = await create_energy_sensor_if_needed(
...
async def create_energy_sensor_if_needed(
    ...
    if (
        ...
        return await create_energy_sensor(
            hass, sensor_config, power_sensor, source_entity
        )
    return None


def attach_energy_sensor_to_power_sensor(
    power_sensor: Entity, energy_sensor: EnergySensor
) -> None:
...
def update_registries(
    ...
    domain_entities = hass.data[DOMAIN][DATA_DOMAIN_ENTITIES].setdefault(
