...
DOMAIN_TO_DEVICE_TYPE = {
    domain: device_type for device_type, domain in DEVICE_TYPE_DOMAIN.items()
}
...
class PowerProfile:
    ...
    async def get_sub_profiles(self) -> list[str]:
        ...
        self._sub_profiles = sorted(
            await self._hass.async_add_executor_job(_get_sub_dirs)
        )  # type: ignore
        return self._sub_profiles
...
class SubProfileSelector:
    ...
    def _build_matchers(self) -> list[SubProfileMatcher]:
        ...
        return [
            self._create_matcher(matcher_config)
