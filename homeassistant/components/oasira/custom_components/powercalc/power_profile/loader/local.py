...
class LocalLoader(Loader):
    def __init__(
        self, hass: HomeAssistant, directory: str, is_custom_directory: bool = False
    ) -> None:
        ...
        if self._is_custom_directory:
            ...
            if not os.path.exists(model_json_path):
                raise LibraryLoadingError(
                    f"model.json not found for manufacturer {_manufacturer} "
                    + f"and model {_model} in path {model_json_path}"
                )

            model_json = await self._hass.async_add_executor_job(
    ...
    async def find_model(self, manufacturer: str, search: set[str]) -> list[str]:
        ...
        if not models:
            _LOGGER.info(
                "Manufacturer does not exist in custom library: %s", _manufacturer
            )
            return []
        ...
        profile = next(
            (models[model] for model in models if model.lower() in search_lower), None
        )
