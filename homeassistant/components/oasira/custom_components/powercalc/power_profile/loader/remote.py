...
from custom_components.powercalc.power_profile.error import (
    LibraryLoadingError,
...
class RemoteLoader(Loader):
    ...
    async def initialize(self) -> None:
        ...
        self.last_update_time = await self.hass.async_add_executor_job(
    ...
    async def load_library_json(self) -> dict[str, Any]:
        ...
        local_path = self.hass.config.path(
            STORAGE_DIR, "powercalc_profiles", "library.json"
        )
        ...
        async def _download_remote_library_json() -> dict[str, Any] | None:
            ...
            async with (
                ...
                session.get(ENDPOINT_LIBRARY) as resp,
            ):
                if resp.status != 200:
                    raise ProfileDownloadError(
                        "Failed to download library.json, unexpected status code"
                    )
                ...
                await self.hass.async_add_executor_job(
                    _save_to_local_storage, await resp.read()
                )
        ...
        try:
            return cast(
                dict[str, Any],
                await self.download_with_retry(_download_remote_library_json),
            )
    ...
    async def get_manufacturer_listing(
        self, device_types: set[DeviceType] | None
    ) -> set[str]:
        ...
        return {
            ...
            if not device_types
            or any(
                device_type in manufacturer.get("device_types", [])
    ...
    async def get_model_listing(
        self, manufacturer: str, device_types: set[DeviceType] | None
    ) -> set[str]:
        ...
        return {
            ...
            if not device_types
            or any(
                device_type in model.get("device_type", [DeviceType.LIGHT])
                for device_type in device_types
            )
        }
    ...
    async def load_model(
        ...
        if await self._needs_update(model_info, model_path, force_update):
            await self._download_profile_with_retry(
                manufacturer.lower(), model, storage_path, model_path
            )

        try:
            json_data = await self._load_model_json(model_path)
        except JSONDecodeError as e:
            return await self._handle_json_decode_error(
                e, manufacturer, model, retry_count
            )
    ...
    def _get_model_info(self, manufacturer: str, model: str) -> dict:
        ...
        if not model_info:
            raise LibraryLoadingError(
                "Model not found in library: %s/%s", manufacturer, model
            )
        return model_info

    async def _needs_update(
        self, model_info: dict, model_path: str, force_update: bool
    ) -> bool:
    ...
    async def _download_profile_with_retry(
        self, manufacturer: str, model: str, storage_path: str, model_path: str
    ) -> None:
    ...
    def get_storage_path(self, manufacturer: str, model: str) -> str:
        ...
        return str(
            self.hass.config.path(
                STORAGE_DIR, "powercalc_profiles", manufacturer, model
            )
        )
    ...
    async def find_model(self, manufacturer: str, search: set[str]) -> list[str]:
        ...
        for model in models:
            ...
            if model_id and (
                model_id in search
                or any(alias in search for alias in model.get("aliases", []))
            ):
    ...
    def _get_remote_modification_time(model_info: dict) -> float:
        ...
        if isinstance(remote_modification_time, str):
            remote_modification_time = datetime.datetime.fromisoformat(
                remote_modification_time
            ).timestamp()
        return remote_modification_time  # type: ignore

    async def download_with_retry(
        self, callback: Callable[[], Coroutine[Any, Any, None | dict[str, Any]]]
    ) -> None | dict[str, Any]:
        ...
        while retry_count < max_retries:
            try:
                ...
                _LOGGER.warning(
                    "Failed to download, retrying... (Attempt %d of %d)",
                    retry_count + 1,
                    max_retries,
                )
    ...
    async def download_profile(
        self, manufacturer: str, model: str, storage_path: str
    ) -> None:
        ...
        async with aiohttp.ClientSession() as session:
            try:
                async with session.get(endpoint) as resp:
                    if resp.status != 200:
                        raise ProfileDownloadError(
                            f"Failed to download profile: {manufacturer}/{model}"
                        )
                    resources = await resp.json()

                await self.hass.async_add_executor_job(
                    lambda: os.makedirs(storage_path, exist_ok=True)
                )  # type: ignore
                ...
                for resource in resources:
                    ...
                    async with session.get(url) as resp:
                        if resp.status != 200:
                            raise ProfileDownloadError(
                                f"Failed to download github URL: {url}"
                            )

                        contents = await resp.read()
                        await self.hass.async_add_executor_job(
                            _save_file, contents, resource.get("path")
                        )  # type: ignore
