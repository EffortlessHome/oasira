...
class ProfileLibrary:
    ...
    def create_loader(hass: HomeAssistant) -> Loader:
        ...
        disable_library_download: bool = bool(
            global_config.get(CONF_DISABLE_LIBRARY_DOWNLOAD, False)
        )
    ...
    async def get_manufacturer_listing(
        self, entity_domain: str | None = None
    ) -> list[str]:
        """Get listing of available manufacturers."""

        device_types = (
            get_device_types_from_domain(entity_domain) if entity_domain else None
        )
        manufacturers = await self._loader.get_manufacturer_listing(device_types)
        return sorted(manufacturers)

    async def get_model_listing(
        self, manufacturer: str, entity_domain: str | None = None
    ) -> list[str]:
        ...
        models = await self._loader.get_model_listing(
            resolved_manufacturer, device_types
        )
    ...
    async def create_power_profile(
        ...
        json_data, directory = await self._load_model_data(
            manufacturer, model, custom_directory
        )
        if linked_profile := json_data.get("linked_lut"):
            linked_manufacturer, linked_model = linked_profile.split("/")
            _, directory = await self._load_model_data(
                linked_manufacturer, linked_model, custom_directory
            )

        return await self._create_power_profile_instance(
            manufacturer, model, directory, json_data
        )
    ...
    async def _load_model_data(
        self, manufacturer: str, model: str, custom_directory: str | None
    ) -> tuple[dict, str]:
        """Load the model data from the appropriate directory."""
        loader = (
            LocalLoader(self._hass, custom_directory, is_custom_directory=True)
            if custom_directory
            else self._loader
        )
    ...
    async def _create_power_profile_instance(
        self, manufacturer: str, model: str, directory: str, json_data: dict
    ) -> PowerProfile:
