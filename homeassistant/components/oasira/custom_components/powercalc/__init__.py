...
async def async_setup(hass: HomeAssistant, config: ConfigType) -> bool:
    ...
    try:
        ...
        _LOGGER.error(
            "problem while cleaning up None entities", exc_info=e
        )  # pragma: no cover
...
def get_global_configuration(hass: HomeAssistant, config: ConfigType) -> ConfigType:
    ...
    global_config_entry = hass.config_entries.async_entry_for_domain_unique_id(
        DOMAIN, ENTRY_GLOBAL_CONFIG_UNIQUE_ID
    )
...
def get_global_gui_configuration(config_entry: ConfigEntry) -> ConfigType:
    ...
    if CONF_FORCE_UPDATE_FREQUENCY in global_config:
        global_config[CONF_FORCE_UPDATE_FREQUENCY] = timedelta(
            seconds=global_config[CONF_FORCE_UPDATE_FREQUENCY]
        )
    if CONF_UTILITY_METER_OFFSET in global_config:
        global_config[CONF_UTILITY_METER_OFFSET] = timedelta(
            days=global_config[CONF_UTILITY_METER_OFFSET]
        )
...
async def apply_global_gui_configuration_changes(
...
async def async_migrate_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    if (
        version <= 2
        and data.get(CONF_SENSOR_TYPE)
        and CONF_CREATE_ENERGY_SENSOR not in data
    ):
    ...
    if version <= 3:
        ...
        if CONF_STATES_TRIGGER in conf_playbook:
            data[CONF_PLAYBOOK][CONF_STATE_TRIGGER] = conf_playbook.pop(
                CONF_STATES_TRIGGER
            )
