...
class LutRegistry:
    ...
    async def get_lookup_dictionary(
        ...
        if lookup_dict is None:
            ...
            csv_file = await self._hass.async_add_executor_job(
                partial(self.get_lut_file, power_profile, color_mode)
            )
            with csv_file:
    ...
    async def get_supported_color_modes(
        self, power_profile: PowerProfile
    ) -> set[ColorMode]:
        """Return the color modes supported by the Profile."""
        cache_key = (
            f"{power_profile.manufacturer}_{power_profile.model}_supported_color_modes"
        )
        supported_color_modes = self._supported_color_modes.get(cache_key)
        if supported_color_modes is None:
            supported_color_modes = set()
            for file in await self._hass.async_add_executor_job(
                os.listdir, power_profile.get_model_directory()
            ):
...
class LutStrategy(PowerCalculationStrategyInterface):
    ...
    async def calculate(self, entity_state: State) -> Decimal | None:
        ...
        if color_mode == ColorMode.HS:
            try:
                hs = (
                    color_temperature_to_hs(attrs[ATTR_COLOR_TEMP])
    ...
    async def get_selected_color_mode(self, attrs: Mapping[str, Any]) -> ColorMode:
        ...
        profile_color_modes = await self._lut_registry.get_supported_color_modes(
            self._profile
        )
