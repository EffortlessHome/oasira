...
class PowerCalculatorStrategyFactory:
    ...
    async def create(
        ...
        strategy_mapping: dict[str, Callable[[], PowerCalculationStrategyInterface]] = {
            CalculationStrategy.LINEAR: lambda: self._create_linear(
                source_entity, config, power_profile
            ),
            CalculationStrategy.FIXED: lambda: self._create_fixed(
                source_entity, config, power_profile
            ),
            CalculationStrategy.LUT: lambda: self._create_lut(
                source_entity, power_profile
            ),
            CalculationStrategy.MULTI_SWITCH: lambda: self._create_multi_switch(
    ...
    def _create_linear(
        ...
        if linear_config is None:
            if power_profile:
                linear_config = power_profile.linear_mode_config or {
                    CONF_MIN_POWER: 0,
                    CONF_MAX_POWER: 0,
                }
            else:
    ...
    def _create_multi_switch(
        self, config: ConfigType, power_profile: PowerProfile | None
    ) -> MultiSwitchStrategy:
