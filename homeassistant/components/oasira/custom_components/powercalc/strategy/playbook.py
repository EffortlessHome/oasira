...
class PlaybookStrategy(PowerCalculationStrategyInterface):
    def __init__(
        ...
        self._states_trigger: dict[str, str] | None = config.get(
            CONF_STATE_TRIGGER, config.get(CONF_STATES_TRIGGER)
        )
    ...
    def _execute_playbook_entry(self) -> None:
        ...
        def _update_power(date_time: datetime) -> None:
            ...
            _LOGGER.debug(
                "playbook %s: Update power %.2f", self._active_playbook.key, self._power
            )  # type: ignore
        ...
        def _load_playbook_entries() -> list[PlaybookEntry]:
            ...
            with open(file_path) as csv_file:
                ...
                for row in csv_reader:
                    ...
                    entries.append(
                        PlaybookEntry(time=float(row[0]), power=Decimal(row[1]))
                    )

                return entries

        playbook_entries = await self._hass.async_add_executor_job(
