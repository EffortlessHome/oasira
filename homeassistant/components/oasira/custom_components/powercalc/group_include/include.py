...
from .filter import FilterOperator, create_composite_filter
...
async def resolve_include_entities(
    ...
    include_non_powercalc: bool = include_config.get(
    ...
    for source_entity in source_entities.values():
        resolved_entities.extend(
            hass.data[DOMAIN][DATA_CONFIGURED_ENTITIES].get(
        ...
        if (
            ...
            if device_class == SensorDeviceClass.POWER:
                resolved_entities.append(
                    RealPowerSensor(
                        source_entity.entity_id, source_entity.unit_of_measurement
                    )
                )
        ...
        if (
            ...
            and source_entity
            and await discovery_manager.is_entity_supported(
                source_entity, None, log_profile_loading_errors=False
            )
        ):
