...
def create_filter(
    ...
    filter_mapping: dict[str, Callable[[], IncludeEntityFilter]] = {
        ...
        CONF_OR: lambda: create_composite_filter(
            filter_config, hass, FilterOperator.OR
        ),  # type: ignore
        CONF_AND: lambda: create_composite_filter(
            filter_config, hass, FilterOperator.AND
        ),  # type: ignore
    }
