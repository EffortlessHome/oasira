...
async def get_power_profile_by_source_entity(
    hass: HomeAssistant, source_entity: SourceEntity
) -> PowerProfile | None:
    ...
    model_info = await discovery_manager.extract_model_info_from_entity(
        source_entity.entity_entry
    )
    if not model_info:
        return None
...
class DiscoveryManager:
    ...
    async def start_discovery(self) -> None:
        ...
        for entry in self.hass.config_entries.async_entries(DOMAIN):
            if entry.unique_id:
                self.initialized_flows.update(
                    {entry.unique_id, str(entry.data.get(CONF_ENTITY_ID))}
                )
        ...
        for entity_entry in list(entity_registry.entities.values()):
            ...
            try:
                model_info = await self.extract_model_info_from_entity(
                    source_entity.entity_entry
                )
                if not model_info:
                    continue
                ...
                self._init_entity_discovery(
                    source_entity, model_info, unique_id, power_profiles, {}
                )
            except Exception:
                _LOGGER.exception(
    ...
    async def discover_entity(
        ...
        for model in models:
            ...
            if not profile:  # pragma: no cover
                continue
            if not await self.is_entity_supported(
                source_entity.entity_entry, model_info, profile
            ):
                continue
    ...
    async def init_wled_flow(
        self, model_info: ModelInfo, source_entity: SourceEntity
    ) -> None:
    ...
    def is_wled_light(model_info: ModelInfo, entity_entry: er.RegistryEntry) -> bool:
        ...
        return (
            ...
            and not re.search(
                "master|segment", str(entity_entry.original_name), flags=re.IGNORECASE
            )
            and not re.search(
                "master|segment", str(entity_entry.entity_id), flags=re.IGNORECASE
            )
        )
        ...
        if not power_profile:
            try:
                power_profile = await get_power_profile(
                    self.hass, {}, model_info, log_errors=log_profile_loading_errors
                )
            except ModelNotSupportedError:
                return False

        return (
            power_profile.is_entity_domain_supported(entity_entry)
            if power_profile
            else False
        )
    ...
    async def extract_model_info_from_entity(
        self, entity_entry: er.RegistryEntry | None
    ) -> ModelInfo | None:
    ...
    def _init_entity_discovery(
        ...
        _LOGGER.debug(
            "%s: Initiating discovery flow, unique_id=%s",
            source_entity.entity_id,
            unique_id,
        )
    ...
    def _extract_entity_ids(
        self, search_dict: dict, found_entity_ids: list[str]
    ) -> None:
    ...
    def _is_already_discovered(
        self, source_entity: SourceEntity, unique_id: str
    ) -> bool:
        ...
        return any(
            unique_id in self.initialized_flows for unique_id in unique_ids_to_check
        )
