...
SCHEMA_UTILITY_METER_OPTIONS = vol.Schema(
    {
        ...
        vol.Optional(
            CONF_UTILITY_METER_NET_CONSUMPTION, default=False
        ): selector.BooleanSelector(),
...
SCHEMA_GLOBAL_CONFIGURATION = vol.Schema(
    {
        ...
        vol.Optional(CONF_POWER_SENSOR_PRECISION): selector.NumberSelector(
            selector.NumberSelectorConfig(
                min=0, max=6, mode=selector.NumberSelectorMode.BOX, step=1
            ),
        ),
        ...
        vol.Optional(
            CONF_IGNORE_UNAVAILABLE_STATE, default=False
        ): selector.BooleanSelector(),
        vol.Optional(
            CONF_INCLUDE_NON_POWERCALC_SENSORS, default=True
        ): selector.BooleanSelector(),
        vol.Optional(
            CONF_DISABLE_EXTENDED_ATTRIBUTES, default=False
        ): selector.BooleanSelector(),
        vol.Optional(
            CONF_DISABLE_LIBRARY_DOWNLOAD, default=False
        ): selector.BooleanSelector(),
        vol.Optional(
...
class PowercalcCommonFlow(ABC, ConfigEntryBaseFlow):
    ...
    async def validate_strategy_config(self) -> dict:
        ...
        strategy_name = CalculationStrategy(
            ...
            or self.selected_profile.calculation_strategy,  # type: ignore
        )
        factory = PowerCalculatorStrategyFactory(self.hass)
        strategy = await factory.create(
            self.sensor_config, strategy_name, self.selected_profile, self.source_entity
        )  # type: ignore
        try:
    ...
    def create_strategy_schema(
        self, strategy: CalculationStrategy, source_entity_id: str
    ) -> vol.Schema:
    ...
    def create_schema_group(
        ...
        member_sensors = [
            ...
            in [SensorType.VIRTUAL_POWER, SensorType.REAL_POWER]
        ...
        schema = vol.Schema(
            {
                ...
                vol.Optional(
                    CONF_FORCE_CALCULATE_GROUP_ENERGY, default=False
                ): selector.BooleanSelector(),
            },
        )
    ...
    async def create_schema_manufacturer(self) -> vol.Schema:
        ...
        return vol.Schema(
            {
                vol.Required(
                    CONF_MANUFACTURER, default=self.sensor_config.get(CONF_MANUFACTURER)
                ): selector.SelectSelector(
                    selector.SelectSelectorConfig(
    ...
    async def create_schema_model(self) -> vol.Schema:
        ...
        models = [
            ...
            for model in await library.get_model_listing(
                manufacturer, self.source_entity.domain
            )  # type: ignore
        ]
        return vol.Schema(
            {
                vol.Required(
                    CONF_MODEL, default=self.sensor_config.get(CONF_MODEL)
                ): selector.SelectSelector(
                    selector.SelectSelectorConfig(
                        options=models,
                        mode=selector.SelectSelectorMode.DROPDOWN,
    ...
    def create_group_selector(
        ...
        options = [
            ...
            and (
                current_entry is None or config_entry.entry_id != current_entry.entry_id
            )
        ]
    ...
    @staticmethod
    def build_daily_energy_config(
        user_input: dict[str, Any], schema: vol.Schema
    ) -> dict[str, Any]:
    ...
    def get_global_powercalc_config(self) -> ConfigType:
        ...
        if isinstance(force_update_frequency, timedelta):
            global_config[CONF_FORCE_UPDATE_FREQUENCY] = (
                force_update_frequency.total_seconds()
            )
    ...
    def get_fixed_power_config_for_smart_switch(
        ...
        self_usage_on = (
            self.selected_profile.fixed_mode_config.get(CONF_POWER, 0)
            if self.selected_profile.fixed_mode_config
            else 0
        )
    ...
    async def async_step_post_library(
        ...
        if (
            self.selected_profile
            and self.selected_profile.calculation_strategy
            == CalculationStrategy.MULTI_SWITCH
        ):
            ...
            self.sensor_config.update(
                {
                    ...
                    CONF_FIXED: self.get_fixed_power_config_for_smart_switch(
                        user_input
                    ),
                },
            )
    ...
    async def async_step_global_configuration_energy(
        ...
        if (
            not bool(self.global_config.get(CONF_CREATE_ENERGY_SENSORS))
    ...
    async def async_step_global_configuration_utility_meter(
        ...
        if (
            not bool(self.global_config.get(CONF_CREATE_UTILITY_METERS))
            or user_input is not None
        ):
...
class PowercalcConfigFlow(PowercalcCommonFlow, ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_integration_discovery(
        ...
        self.skip_advanced_step = (
            True  # We don't want to ask advanced options when discovered
        )
        ...
        if DISCOVERY_POWER_PROFILES in discovery_info:
            ...
            self.discovered_profiles = {
                profile.unique_id: profile for profile in power_profiles
            }
    ...
    async def async_step_global_configuration(
        self, user_input: dict[str, Any] | None = None
    ) -> FlowResult:
        """Handle the global configuration step."""
    ...
    async def async_step_daily_energy(
        ...
        if user_input is not None and not errors:
            ...
            self.sensor_config.update(
                self.build_daily_energy_config(user_input, schema)
            )
            if self.sensor_config.get(CONF_CREATE_UTILITY_METERS):
                return await self.async_step_utility_meter_options()
            return self.persist_config_entry()
    ...
    async def async_step_domain_group(
        self, user_input: dict[str, Any] | None = None
    ) -> FlowResult:
    ...
    async def async_step_subtract_group(
        self, user_input: dict[str, Any] | None = None
    ) -> FlowResult:
    ...
    async def async_step_library(
        ...
        if user_input is not None:
            if (
                user_input.get(CONF_CONFIRM_AUTODISCOVERED_MODEL)
        ...
        if (
            ...
            self.selected_profile = await get_power_profile_by_source_entity(
                self.hass, self.source_entity
            )
...
class PowercalcOptionsFlow(PowercalcCommonFlow, OptionsFlow):
    ...
    def __init__(self, config_entry: ConfigEntry) -> None:
        ...
        self.sensor_type: SensorType = (
            self.sensor_config.get(CONF_SENSOR_TYPE) or SensorType.VIRTUAL_POWER
        )
    ...
    async def async_step_init(
        ...
        if self.config_entry.unique_id == ENTRY_GLOBAL_CONFIG_UNIQUE_ID:
            ...
            return self.async_show_menu(
                step_id=Steps.INIT, menu_options=self.build_global_config_menu()
            )
    ...
    async def async_step_basic_options(
        ...
        return await self.async_handle_options_step(
            user_input, schema, Steps.BASIC_OPTIONS
        )
    ...
    async def async_step_advanced_options(
        ...
        return await self.async_handle_options_step(
            user_input, schema, Steps.ADVANCED_OPTIONS
        )
    ...
    async def async_step_utility_meter_options(
        ...
        return await self.async_handle_options_step(
            user_input, schema, Steps.UTILITY_METER_OPTIONS
        )
    ...
    async def async_step_daily_energy(
        ...
        return await self.async_handle_options_step(
            user_input, schema, Steps.DAILY_ENERGY
        )
    ...
    async def async_step_real_power(
        ...
        return await self.async_handle_options_step(
            user_input, schema, Steps.REAL_POWER
        )
    ...
    async def async_step_subtract_group(
        ...
        return await self.async_handle_options_step(
            user_input, schema, Steps.SUBTRACT_GROUP
        )
    ...
    async def async_step_library_options(
        self, user_input: dict[str, Any] | None = None
    ) -> FlowResult:
        """Handle the basic options flow."""
        self.is_library_flow = True
    ...
    async def async_handle_strategy_options_step(
        self, user_input: dict[str, Any] | None = None
    ) -> FlowResult:
    ...
    async def async_handle_options_step(
        self, user_input: dict[str, Any] | None, schema: vol.Schema, step: Steps
    ) -> FlowResult:
    ...
    def persist_config_entry(self) -> FlowResult:
        ...
        data = (
            self.config_entry.unique_id == ENTRY_GLOBAL_CONFIG_UNIQUE_ID
    ...
    async def process_all_options(
        ...
        if (
            self.sensor_type == SensorType.DAILY_ENERGY
            and current_step == Steps.DAILY_ENERGY
        ):
            self.sensor_config.update(
                self.build_daily_energy_config(user_input, SCHEMA_DAILY_ENERGY_OPTIONS)
            )
