...
async def create_virtual_power_sensor(
    ...
    try:
        ...
        standby_power, standby_power_on = _get_standby_power(
            sensor_config, power_profile
        )
...
async def _get_power_profile(
    ...
    try:
        ...
        if power_profile and power_profile.sub_profile_select:
            await _select_sub_profile(
                hass, power_profile, power_profile.sub_profile_select, source_entity
            )
...
class VirtualPowerSensor(SensorEntity, PowerSensor):
    ...
    def __init__(
        ...
        self._update_frequency: timedelta = sensor_config.get(
            CONF_FORCE_UPDATE_FREQUENCY
        )  # type: ignore
        self._multiply_factor = sensor_config.get(CONF_MULTIPLY_FACTOR)
        self._multiply_factor_standby = bool(
            sensor_config.get(CONF_MULTIPLY_FACTOR_STANDBY, False)
        )
        self._ignore_unavailable_state = bool(
            sensor_config.get(CONF_IGNORE_UNAVAILABLE_STATE, False)
        )
    ...
    def init_calculation_enabled_condition(self) -> None:
        ...
        template: Template | str = self._sensor_config.get(
            CONF_CALCULATION_ENABLED_CONDITION
        )  # type: ignore
    ...
    def _has_valid_state(self, state: State) -> bool:
        ...
        return self._ignore_unavailable_state or state.state not in [
            STATE_UNAVAILABLE,
            STATE_UNKNOWN,
        ]
    ...
    async def calculate_standby_power(self, state: State) -> Decimal:
        ...
        if self._strategy_instance.can_calculate_standby():
            standby_power = (
                await self._strategy_instance.calculate(state) or self._standby_power
            )
