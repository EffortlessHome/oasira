...
async def create_group_sensors_yaml(
    ...
    return await create_group_sensors_custom(
        hass, group_name, sensor_config, set(power_sensor_ids), set(energy_sensor_ids)
    )
...
async def create_group_sensors_gui(
    ...
    power_sensor_ids = await resolve_entity_ids_recursively(
        hass, entry, SensorDeviceClass.POWER
    )

    energy_sensor_ids = await resolve_entity_ids_recursively(
        hass, entry, SensorDeviceClass.ENERGY
    )

    return await create_group_sensors_custom(
        hass, group_name, sensor_config, power_sensor_ids, energy_sensor_ids
    )
...
async def create_group_sensors_custom(
    ...
    group_type: GroupType = GroupType(
        sensor_config.get(CONF_GROUP_TYPE, GroupType.CUSTOM)
    )
    ...
    async def add_area_entities() -> None:
        ...
        resolved_area_entities, _ = await resolve_include_entities(
            hass,
            {
                ...
                CONF_INCLUDE_NON_POWERCALC_SENSORS: entry.data.get(
                    CONF_INCLUDE_NON_POWERCALC_SENSORS
                ),
            },
        )
...
def create_grouped_power_sensor(
    ...
    return GroupedPowerSensor(
        ...
        rounding_digits=int(
            sensor_config.get(CONF_POWER_SENSOR_PRECISION)
...
def create_grouped_energy_sensor(
    ...
    should_create_riemann = bool(
        sensor_config.get(CONF_FORCE_CALCULATE_GROUP_ENERGY, False)
    )
...
class GroupedSensor(BaseEntity, RestoreSensor, SensorEntity):
    ...
    def __init__(
        ...
        self._ignore_unavailable_state = bool(
            self._sensor_config.get(CONF_IGNORE_UNAVAILABLE_STATE)
        )
    ...
    async def init_domain_group(self) -> None:
        ...
        self._entities = set(
            {entity for entity in entities if entity not in excluded_entities}
        )
    ...
    async def on_start(self, _: Any) -> None:  # noqa
        ...
        if not self._entities:
            _LOGGER.warning(
                "No entities for group sensor %s, setting to unavailable",
                self.entity_id,
            )
    ...
    def _get_state_value_in_native_unit(self, state: State) -> Decimal:
        ...
        if (
            ...
            convert = converter.converter_factory(
...
class GroupedPowerSensor(GroupedSensor, PowerSensor):
    ...
    def calculate_initial_state(
        ...
        self._states = {
            state.entity_id: self._get_state_value_in_native_unit(state)
...
class GroupedEnergySensor(GroupedSensor, EnergySensor):
    ...
    def calculate_initial_state(
        ...
        """
        group_sum = (
            Decimal(self._native_value_exact)
            if self._native_value_exact
            else Decimal(0)
        )
    ...
    def calculate_delta(self, state: State) -> Decimal:
        ...
        prev_state_value = (
            self._get_state_value_in_native_unit(prev_state)
            if prev_state
            else Decimal(0)
        )
        self._prev_state_store.set_entity_state(
