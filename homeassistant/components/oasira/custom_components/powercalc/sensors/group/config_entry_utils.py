...
async def add_to_associated_group(
    ...
    if not group_entry and len(group_entry_id) != 32:
        group_entry = hass.config_entries.async_entry_for_domain_unique_id(
            DOMAIN, group_entry_id
        )
        if not group_entry:
...
def get_entries_having_subgroup(
    hass: HomeAssistant, subgroup_entry: ConfigEntry
) -> list[ConfigEntry]:
...
def get_groups_having_member(
    hass: HomeAssistant, member_entry: ConfigEntry
) -> list[ConfigEntry]:
