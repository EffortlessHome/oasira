...
async def create_utility_meters(
    ...
    for meter_type in meter_types:
        unique_id = (
            f"{energy_sensor.unique_id}_{meter_type}"
...
def should_create_utility_meter(
    ...
    return not (
        existing_entity_id and hass.states.get(existing_entity_id)
    )  # pragma: no cover
...
async def create_utility_meter(
    ...
    params = {
        ...
        "net_consumption": bool(
            sensor_config.get(CONF_UTILITY_METER_NET_CONSUMPTION, False)
        ),
        "tariff": tariff,
    ...
    if "sensor_always_available" in signature.parameters:
        params["sensor_always_available"] = (
            sensor_config.get(CONF_IGNORE_UNAVAILABLE_STATE) or False
        )
