...
async def create_energy_sensor(
    ...
    return await _create_virtual_energy_sensor(
...
async def _get_related_energy_sensor(
    ...
    if CONF_POWER_SENSOR_ID not in sensor_config or not isinstance(
...
async def _create_virtual_energy_sensor(
    ...
    unique_id = (
        f"{power_sensor.unique_id}_energy"
        if power_sensor.unique_id is not None
        else None
    )
...
class VirtualEnergySensor(IntegrationSensor, EnergySensor):
    ...
    def __init__(
        ...
        integration_method: str = sensor_config.get(
