...
class PresenceSimulationConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_user(self, info=None):
        ...
        data_schema = {
            vol.Required(
                "switch", description={"suggested_value": "Choose a unique name"}
            ): str,
        ...
        all_entities = self.hass.states.async_entity_ids()
        switch_id = (
            SWITCH_PLATFORM + "." + re.sub("[^0-9a-zA-Z]", "_", info["switch"].lower())
        )
        if switch_id in all_entities:
            _LOGGER.error(
                "Entity name is already taken, please change the name of the switch"
            )
...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_init(self, info=None):
        ...
        data_schema = {
            ...
            vol.Required(
                "entities", default=self.config_entry.data["entities"].split(",")
            ): SelectSelector(
                SelectSelectorConfig(
                    options=all_entities,
                    multiple=True,
