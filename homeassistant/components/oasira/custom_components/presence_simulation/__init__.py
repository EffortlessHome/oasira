...
async def async_setup_entry(hass, entry):
    ...
    hass.async_create_task(
        hass.config_entries.async_forward_entry_setups(entry, [SWITCH_PLATFORM])
    )
    ...
    async def stop_presence_simulation(err=None, restart=False, switch_id=None):
        ...
        if not restart:
            ...
            scene = hass.states.get(
                SCENE_PLATFORM + "." + switch_id.replace(".", "_") + "_" + RESTORE_SCENE
            )
            if scene is not None and entity.restore:
                ...
                try:
                    await hass.services.async_call(
                        "scene", "turn_on", service_data, blocking=False
                    )
                except Exception:
    ...
    async def handle_stop_presence_simulation(call, restart=False, switch_id=None):
        ...
        if (
            call is not None
        ):  # if we are here, it is a call of the service, or a restart at the end of a cycle
        ...
        if is_running(switch_id):
            ...
            _LOGGER.warning(
                "Presence simulation switch %s is not on, can't be turned off",
                switch_id,
            )
    ...
    async def async_expand_entities(entities):
        ...
        for entity in entities:
            ...
            elif "entity_id" in hass.states.get(entity).attributes:
    ...
    async def handle_presence_simulation(call, restart=False, switch_id=None):
        ...
        if (
            call is not None
        ):  # if we are here, it is a call of the service, or a restart at the end of a cycle, or a restore after a HA restart
            # get the switch entity
        ...
        if not restart:
            ...
            try:
                ...
                try:
                    presence_timezone = await hass.async_add_executor_job(
                        pytz.timezone, hass.config.time_zone
                    )
        ...
        _LOGGER.debug(
            "Getting the historic from %s for %s", minus_delta, expanded_entities
        )
        await get_instance(hass).async_add_executor_job(
            handle_presence_simulation_sync,
            hass,
            call,
            minus_delta,
            expanded_entities,
    ...
    def handle_presence_simulation_sync(
        hass, call, minus_delta, expanded_entities, switch_id
    ):
        ...
        for entity_id in dic:
            ...
            hass.create_task(
                simulate_single_entity(
                    switch_id, entity_id, dic[entity_id], entity.delta, entity.random
                )
            )
    ...
    async def restart_presence_simulation(call, switch_id=None):
        ...
        start_plus_delta = datetime.now(UTC) + timedelta(entity.delta)
        while is_running(switch_id):
            # sleep until the 'delay' is passed
            secs_left = (start_plus_delta - datetime.now(UTC)).total_seconds()
            if secs_left <= 0:
                break
        ...
        if is_running(switch_id):
            _LOGGER.debug(
                "%s has passed, presence simulation is relaunched", entity.delta
            )
            # Call to stop needed to avoid the start to do nothing since already running
            await handle_stop_presence_simulation(
                call, restart=True, switch_id=switch_id
            )
    ...
    async def simulate_single_entity(switch_id, entity_id, hist, delta, random_val):
        ...
        for idx, state in enumerate(
            hist
        ):  # hypothsis: states are ordered chronologically
            ...
            _LOGGER.debug(
                "Switch of %s foreseen at %s",
                entity_id,
                _last_updated + timedelta(delta),
            )
            # get the switch entity
            entity = hass.data[DOMAIN][SWITCH_PLATFORM][switch_id]
            ...
            if idx > 0:
                _LOGGER.debug(
                    "Randomize the event within a range of +/- %s sec", random_val
                )
                random_delta = random.uniform(
                ...
                initial_secs_left = (target_time - datetime.now(UTC)).total_seconds()
                if initial_secs_left < MIN_DELAY and random_val > 0:
                    ...
                    target_time = datetime.now(UTC) + timedelta(
                        MIN_DELAY / 60 / 60 / 24
                    )
                    _LOGGER.debug("target_time after %s", target_time)
                else:
                    _LOGGER.debug(
                        "initial_secs_left %s, target_time %s",
                        initial_secs_left,
                        target_time,
                    )
            ...
            while is_running(switch_id):
                ...
                secs_left = (target_time - datetime.now(UTC)).total_seconds()
                if secs_left <= 0:
                    break
                await asyncio.sleep(min(secs_left, entity.interval))
    ...
    async def update_entity(entity_id, state, unavailable_as_off):
        ...
        if domain == "light":
            ...
            if (
                ...
                _LOGGER.debug(
                    "Got attribute brightness: %s", state.attributes["brightness"]
                )
            ...
            if (
                "color_mode" in state.attributes
                and state.attributes["color_mode"] is not None
            ):
                _LOGGER.debug(
                    "Got attribute color_mode: %s", state.attributes["color_mode"]
                )
            ...
            if (
                state.state == "on"
                or state.state == "off"
                or (state.state == "unavailable" and unavailable_as_off)
            ):
                s = "on" if state.state == "on" else "off"
                await hass.services.async_call(
                    "light", "turn_" + s, service_data, blocking=False
                )
                event_data = {
                    "entity_id": entity_id,
                    "service": "light.turn_" + s,
                    "service_data": service_data,
                }
            else:
                _LOGGER.debug(
                    "State in neither on nor off (is %s), do nothing", state.state
                )
            ...
            if state.state == "closed" or (
                ...
                if "current_position" in state.attributes:
                    ...
                    await hass.services.async_call(
                        "cover", "set_cover_position", service_data, blocking=blocking
                    )
                    event_data = {
                        "entity_id": entity_id,
                    ...
                    await hass.services.async_call(
                        "cover", "open_cover", service_data, blocking=blocking
                    )
                    event_data = {
                        "entity_id": entity_id,
            ...
            if (
                state.state in ["closed", "open"]
            ):  # nothing to do if closing or opening. Wait for the status to be 'stabilized'
                if "current_tilt_position" in state.attributes:
                    service_data["tilt_position"] = state.attributes[
                    ...
                    await hass.services.async_call(
                        "cover", "set_cover_tilt_position", service_data, blocking=False
                    )
                    event_data = {
                        "entity_id": entity_id,
            ...
            if state.state == "playing":
                await hass.services.async_call(
                    "media_player", "media_play", service_data, blocking=False
                )
                event_data = {
                    "entity_id": entity_id,
                ...
                state.state != "unavailable" or unavailable_as_off
            ):  # idle, paused, off
                await hass.services.async_call(
                    "media_player", "media_stop", service_data, blocking=False
                )
                event_data = {
                    "entity_id": entity_id,
            ...
            if (
                state.state == "on"
                or state.state == "off"
                or (state.state == "unavailable_as_off" and unavailable_as_off)
            ):
                s = "on" if state.state == "on" else "off"
                await hass.services.async_call(
                    "homeassistant", "turn_" + s, service_data, blocking=False
                )
                event_data = {
                    "entity_id": entity_id,
                    "service": "homeassistant.turn_" + s,
                    "service_data": service_data,
                }
            else:
                _LOGGER.debug(
    ...
    hass.bus.async_listen_once(
        EVENT_HOMEASSISTANT_STARTED, launch_simulation_after_restart
    )
...
async def update_listener(hass, entry):
    ...
    _LOGGER.debug(
        "Updating listener entry id %s, %s, %s",
        entry.entry_id,
        entry.data,
        entry.options,
    )
    ...
    if len(entry.options) > 0:
        switch_id = (
            SWITCH_PLATFORM
            + "."
            + re.sub("[^0-9a-zA-Z]", "_", entry.data["switch"].lower())
        )
        try:
            entity = hass.data[DOMAIN][SWITCH_PLATFORM][switch_id]
        except Exception:
