...
class AIAutomationCoordinator(DataUpdateCoordinator):
    ...
    async def process_with_openai(self, prompt):
        ...
        try:
            ...
            _LOGGER.debug(
                "Making OpenAI API request with model %s and max_tokens %d",
                model,
                max_tokens,
            )
            ...
            async with self.session.post(
                ENDPOINT_OPENAI, headers=headers, json=data
            ) as response:
    ...
    async def process_with_anthropic(self, prompt):
        ...
        try:
            ...
            _LOGGER.debug(
                "Making Anthropic API request with model %s and max_tokens %d",
                model,
                max_tokens,
            )
            ...
            async with self.session.post(
                ENDPOINT_ANTHROPIC, headers=headers, json=data
            ) as response:
                if response.status != 200:
                    error_text = await response.text()
    ...
    async def process_with_groq(self, prompt):
        ...
        try:
            ...
            _LOGGER.debug(
                "Making Groq API request with model %s and max_tokens %d",
                model,
                max_tokens,
            )
            ...
            async with self.session.post(
                ENDPOINT_GROQ, headers=headers, json=data
            ) as response:
                if response.status != 200:
                    error_text = await response.text()
    ...
    async def process_with_localai(self, prompt):
        ...
        try:
            ...
            _LOGGER.debug(
                "Making LocalAI API request to %s with model %s and max_tokens %d",
                endpoint,
                model,
                max_tokens,
            )
    ...
    async def process_with_ollama(self, prompt):
        ...
        try:
            ...
            _LOGGER.debug(
                "Making Ollama API request to %s with model %s and max_tokens %d",
                endpoint,
                model,
                max_tokens,
            )
    ...
    async def process_with_custom_openai(self, prompt):
        ...
        try:
            ...
            _LOGGER.debug(
                "Making Custom OpenAI API request to %s with model %s and max_tokens %d",
                endpoint,
                model,
                max_tokens,
            )
