...
async def async_migrate_entry(hass: HomeAssistant, config_entry: ConfigEntry) -> bool:
    ...
    if config_entry.version > CONFIG_VERSION:
        ...
        _LOGGER.debug(
            "Downgrading config entry from version %s to version %s",
        ...
        _LOGGER.debug(
            "Downgrade migration to version %s successful", config_entry.version
        )
        return False
    ...
    if config_entry.version < CONFIG_VERSION:
        _LOGGER.debug(
            f"Migrating config entry from version {config_entry.version} to version {CONFIG_VERSION}"
        )
        new_data = {**config_entry.data}
