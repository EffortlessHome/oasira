...
@attr.s(slots=True, frozen=True)
class effortlesshomeTriggerEntry:
...
class AutomationEntry:
    ...
    triggers = attr.ib(type=[effortlesshomeTriggerEntry], default=[])
...
def parse_automation_entry(data: dict):
    def create_trigger_entity(config: dict):
        if "event" in config:
            return effortlesshomeTriggerEntry(**config)
    ...
    if "actions" in data:
        output["actions"] = [ActionEntry(**el) for el in data["actions"]]
...
class effortlesshomeStorage:
    """Class to hold effortlesshome configuration data."""
    ...
    async def async_factory_default(self) -> None:
    ...
    def async_schedule_save(self) -> None:
        """Schedule saving the registry of effortlesshome."""
        self._store.async_delay_save(self._data_to_save, SAVE_DELAY)

    async def async_save(self) -> None:
        """Save the registry of effortlesshome."""
        await self._store.async_save(self._data_to_save())

    @callback
    def _data_to_save(self) -> dict:
        """Return data for the registry for effortlesshome to store in a file."""
        store_data = {
    ...
    async def async_delete(self) -> None:
        """Delete config."""
        _LOGGER.warning("Removing effortlesshome configuration data!")
...
async def async_get_registry(hass: HomeAssistant) -> effortlesshomeStorage:
    """Return effortlesshome storage instance."""
    task = hass.data.get(DATA_REGISTRY)

    if task is None:

        async def _load_reg() -> effortlesshomeStorage:
            registry = effortlesshomeStorage(hass)
            await registry.async_load()
            return registry

        task = hass.data[DATA_REGISTRY] = hass.async_create_task(_load_reg())

    return cast(effortlesshomeStorage, await task)
