...
class AutoLights:
    ...
    def __init__(self, auto_area) -> None:
        ...
        # Config
        self.illuminance_threshold = 0
        self.is_sleeping_area = False
        self.excluded_light_entities = []

        # Entities
        self.sleep_mode_entity_id = f"{SLEEP_MODE_SWITCH_ENTITY_PREFIX}{
                slugify(self.auto_area.area_name)}"
        self.presence_entity_id = f"{PRESENCE_BINARY_SENSOR_ENTITY_PREFIX}{
                slugify(self.auto_area.area_name)}"
        self.illuminance_entity_id = f"{ILLUMINANCE_SENSOR_ENTITY_PREFIX}{
                slugify(self.auto_area.area_name)}"
        self.light_group_entity_id = f"{LIGHT_GROUP_ENTITY_PREFIX}{
                slugify(self.auto_area.area_name)}"
    ...
    async def initialize(self):
        ...
        _LOGGER.debug(
            "AutoLights %s %s", self.presence_entity_id, self.illuminance_entity_id
        )
    ...
    async def handle_presence_state_change(self, event: Event[EventStateChangedData]):
        ...
        _LOGGER.debug("handle_presence_state_change")
