...
from . import const
from .alarm_control_panel import effortlesshomeBaseEntity
from .helpers import (
...
class AutomationHandler:
    def __init__(self, hass: HomeAssistant) -> None:
        ...
        self._subscriptions.append(
            async_dispatcher_connect(
                hass, "effortlesshome_automations_updated", async_update_config
            )
        )
        async_update_config()
        ...
        async def async_alarm_state_changed(
            ...
            _LOGGER.debug(
                f"state of {alarm_entity.entity_id} is updated from {old_state} to {new_state}"
            )
        ...
        async def async_handle_event(
            ...
            _LOGGER.debug(f"{alarm_entity.entity_id} has failed to arm")
    ...
    async def async_execute_automation(
        self, automation_id: str, alarm_entity: effortlesshomeBaseEntity
    ) -> None:
        # automation is a dict of AutomationEntry
        _LOGGER.debug(f"Executing automation {automation_id}")
        ...
        for action in actions:
            try:
                ...
                _LOGGER.exception(
                    f"Execution of action {automation_id} failed, reason: {e}"
                )
    ...
    async def replace_wildcards_in_string(
        self, input: str, alarm_entity: effortlesshomeBaseEntity
    ):
    ...
    async def async_get_open_sensor_string(
        ...
        if state == STATE_OPEN:
            translation_key = (
                "component.binary_sensor.device_automation.condition_type.{}".format(
