...
from .const import PRESENCE_LOCK_SWITCH_PREFIX
...
class PresenceLockSwitch(SwitchEntity):
    ...
    def unique_id(self) -> str | None:
        """Return a unique ID."""
        return f"{self.auto_area.area_id}_presence_lock"
    ...
    def turn_on(self, **kwargs) -> None:
        """Turn on switch."""
        _LOGGER.info("%s: Presence lock turned on", self.auto_area.area_name)
        self._is_on = True
        self.schedule_update_ha_state()

    def turn_off(self, **kwargs):
        """Turn off switch."""
        _LOGGER.info("%s: Presence lock turned off", self.auto_area.area_name)
        self._is_on = False
        self.schedule_update_ha_state()
